...
from uploads.models import CSVFile
...
def select_file(request, file_id):
    csv_file = get_object_or_404(CSVFile, id= file_id)
    request.session['selected_file_path'] = csv_file.file.path
        # load CSV into a Pandas Dataframe
    try:
        df = pd.read_csv(request.session.get('selected_file_path'))
...
def generated_graphs(request):
    if request.method == 'POST':
        ...
        return render(request, 'selected_file_graphs.html', context)
    return redirect('upload_file')
